...
import {ActionCount} from "../../../db/models/ActionCounts";
...
export const prepareDetailedEmbed = async (
  ...
  if (mentions.length === 1) {
    ...
    let user = await ActionCount.findOne({
      where: {userID: message.author.id},
    });
    if (!user) user = await ActionCount.create({userID: message.author.id});
    const actionCounts = JSON.parse(user[action] as string) as Map<Snowflake,
        number>;
    actionCounts.set(
        mentions[0].id,
        (actionCounts.get(mentions[0].id) || 0) + 1
    )
    const count = actionCounts.get(mentions[0].id)
    user[action] = JSON.stringify(actionCounts);
    await user.save();
    const currentActionCount = count;
